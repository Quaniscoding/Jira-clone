...
export default function TaskItem({ item, onDeleteTask, onClickTask, listProjectDetail, getTypeLabel, getPriorityLabel, useEffect, taskId }) {
    ...
    useEffect(() => {
        if (listUserAssign.length > 0) {
            Promise.all(listUserAssign.map(userId => dispatch(callGetListUserById(userId))))
                .then((responses) => {
            ...
            <div className={"bg-gray-200 w-full h-full p-2 rounded d-flex flex-column"}>
                ...
                <div>
                    {item.listTaskDetail.map((task, index) => {
                        const taskType = task.typeId === '665c5b7ef5b71f8020dc3c67' ? (
                                ...
                                <div className="mt-2 bg-white p-3 shadow rounded">
                                    <div className="d-flex justify-content-between">
                                        <div className="d-flex align-items-center justify-content-center">
                                            <span className='d-flex align-items-center pl-2'>Task type: {taskType}</span>
                                            <b className='pl-2'>{getTypeLabel(task.typeId)}</b>
                                        </div>
                                        <div className="text-right mt-2">
                                            <button className="btn bg-danger text-white font-bold" onClick={(e) => handleDeleteTask(e, task)}>Delete</button>
                                        </div>
                                    </div>
                                    <div className="w-full bg-white rounded py-3 px-2 mt-1" role="button">
                                        <div className="row row-gap-0">
                                            <div className="col-8">
                                                <div className="pl-2 mb-2">Description: {task.description}</div>
                                                <span className="text-xl rounded px-1 pb-0.5 text-orange-700 border border-orange-700">Priority: {getPriorityLabel(task.priorityId)}</span>
                                            </div>
                                            <div className="col-4">
